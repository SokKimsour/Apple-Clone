    ...
    <header className="h-[44px] fixed top-0 z-50 w-full bg-white/80 backdrop-blur-md shadow-sm transition-all duration-300">
      <nav className="mx-auto flex items-center justify-between h-full max-w-[980px] px-4 text-black relative z-[60]">
        ...
        <NavLink to="/" className="flex items-center justify-center z-[60]" onClick={() => setIsMobileMenuOpen(false)}>
          <svg className="w-[44px] h-[44px]" viewBox="0 0 16 44" fill="#333232">
          ...
          {navItems.map((item) => (
            <li key={item} className="group flex items-center h-full">
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center justify-center h-full text-[13px] font-normal no-underline transition-opacity duration-200 ${isActive ? "opacity-100" : "opacity-80 hover:opacity-100"
                  }`
                }
                style={{ color: "black" }}
              >
        ...
        <div className="flex items-center gap-4 min-[830px]:gap-8 z-[60]">
          ...
          <button
            className="min-[830px]:hidden flex flex-col justify-center gap-[5px] w-[44px] h-[44px] cursor-pointer z-[60]"
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
          >
            <span
              className={`block w-full h-[1px] bg-black transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "rotate-45 translate-y-[3px]" : ""
                }`}
            />
            <span
              className={`block w-full h-[1px] bg-black transition-all duration-300 ease-in-out ${isMobileMenuOpen ? "-rotate-45 -translate-y-[3px]" : ""
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-white z-[50] pt-[60px] px-8 transition-all duration-500 ease-[cubic-bezier(0.4,0,0.2,1)] ${isMobileMenuOpen
      ...
      >
        <ul className="flex flex-col gap-4 text-2xl font-semibold text-gray-800">
          {navItems.map((item, index) => (
            <li
              key={item}
              className={`border-b border-gray-100 pb-2 transition-all duration-500 delay-[${index * 50}ms] ${isMobileMenuOpen ? "translate-x-0 opacity-100" : "-translate-x-4 opacity-0"
                }`}
            >
              <NavLink
                to={`/${item.toLowerCase().replace(/\s|&/g, "")}`}
